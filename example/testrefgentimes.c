/**
	...
	@palette
	NO
	^^^ If your object is a U/I object and you want it to have a palette in the object explorer, you need to put YES.
	However, this implies that you must put in the package (e.g. in the "media" folder) a file named like the object, with extension .svg,
	Containing the object icon


	@category 
	bach, bach objects, bach llll

	^^^ Add a series of categories concerning the object. Separate them with commas.


	@keywords
	multiply, signal, test, reference file, random

	^^^ Add a series of keywords, if you wish
	...
	@illustration on @caption Example of usage

	^^^ If you have an image exemplifying the behavior of your object, you can set it as example by using the "illustration" tag, and you
	can give an optional caption by adding a "caption" tag on the same line (caption must be on a single line).
	The image must be a PNG image, named exactly as the object name (the "name" tag, NOT the "realname" tag), in our case: "testrefgen*.png",
	and must be placed inside a "images" subfolder, placed at the same level of the generated xml files.
	Doctor Max does not move nor produce any image, thus you should place the image at the correct location yourself.
	...
	- As a good rule, all "digest" tags must be short, without the ending period, and without third person's "s";
...
{	
	...
	// Suppose now that you have multiple objects in your library, and that some object share some common attributes.
	// You might want to declare them in another file (which is not provided here), so you might want to write something like:
	add_foo_common_attribute(c);
	...
	// You can exclude some object from having the previous attribute documented by using @exclude. 
	// If @exclude is inside a substitution file (as the one recalled by add_foo_common_attribute)
	// you might want to exclude more than one object; in this case, you might list them as "@exclude firstobj secondobj..."
	// If you need to exclude ALL objects, use "@exclude all".
